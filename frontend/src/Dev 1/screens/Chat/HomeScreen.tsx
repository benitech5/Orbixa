...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      setError("");
      try {
        // For now, use mock data since getUserGroups might not be implemented
        const mockGroupsData = mockChats.map((chat: any) => ({
          ...chat,
          isImportant: Math.random() > 0.7, // Randomly mark some as important
          isPinned: Math.random() > 0.8, // Randomly mark some as pinned
          unreadCount: chat.unread || Math.floor(Math.random() * 5), // Random unread count
          avatar: `https://via.placeholder.com/50?text=${chat.name.charAt(0)}`,
        }));
  ...
  const shouldShowPlusIcon = (storyName: string) => {
    return storyName === "My Story" && (getUserStories ? getUserStories().length === 0 : true);
  };
    ...
    >
      ...
      <View style={styles.chatTextContainer}>
        <View style={styles.chatHeader}>
          ...
          <Text style={[styles.time, { color: "#999" }]}>{item.time || "09:00"}</Text>
        </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (story.name === "My Story") {
                  const userStories = getUserStories ? getUserStories() : [];
